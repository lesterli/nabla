...
| 组件 | 职责 | 集成方式 |
|------|------|---------|
| **Docling** | 主 parser：layout understanding、table structure、reading order | HTTP client → docling-serve（`localhost:5001`），fallback 到 pdf-extract |
| **OCRmyPDF** | OCR fallback：给扫描件加 text layer | sidecar |
| **pdfium-render** | 桌面阅读器：页面渲染、文本/图片提取 | Rust binding（需绑定 Pdfium，注意线程安全） |

Docling 是 Python 生态，通过 HTTP API（docling-serve）与 Rust pipeline 解耦。`AutoParser` 自动探测 docling-serve 可用性，不可用时 fallback 到 `pdf-extract`（纯 Rust）。Docling 输出 `StructuredDocument`，包含 heading/paragraph/table/figure 等结构化元素，驱动结构感知的 chunking 和 summarization。
